...
result=load([output 'fitted_results.mat']);
...
for ifield=1:length(fields)
    if ~isempty(fields{ifield})
        if isstr(data_file) && ~isempty(data_file)
            save_nii_v2((mean(result.fitted(:,:,:,:,ifield),4)),[output fields{ifield}],data_file,64);
        end
    end
end
...
fcsf = (mean(result.fitted(:,:,:,:,strcmp(fields,'fcss')),4));
if isempty(fcsf), fcsf=zeros(dims(1:3)); end
fh(~fh)=1;
if isstr(data_file) && ~isempty(data_file)
    save_nii_v2(ones(dims(1:3)) - fh - fcsf,[output 'fr'],data_file);
end
